...
pub fn solve(input: &String) {
    let mut plant_map: Vec<Vec<(char, i32)>> = input.lines().map(|line| {
    ...
    for iter_x in 0..rows {
        for iter_y in 0..cols {
            let (iter_node_type, iter_node_region) = plant_map.get(iter_x).and_then(|row| row.get(iter_y)).unwrap().to_owned();
            if iter_node_region == 0 {
                ...
                while to_process.len() != 0 {
                    ...
                    curr_region_area += 1;
                    region_map.get_mut(&current_region_id).unwrap().push((next_to_process_coord.0, next_to_process_coord.1));

                    if let Some(n) = plant_map.get_mut(next_to_process_coord.0).and_then(|row| row.get_mut(next_to_process_coord.1)) {
                        n.1 = current_region_id;
                        curr_region_border += 4;
                    }

                    for Coordinate(del_x, del_y) in STRAIGHT_DIRECTIONS.iter().map(|d| get_direction_vector(*d)) {
                        let to_check_x: i32 = i32::try_from(next_to_process_coord.0).unwrap() + i32::from(del_x);
                        let to_check_y: i32 = i32::try_from(next_to_process_coord.1).unwrap() + i32::from(del_y);
                        ...
                        if let Some(n) = plant_map.get(to_check_x_u).and_then(|row| row.get(to_check_y_u)) { 
                            if n.0 == type_to_find {
                                let x = (to_check_x.try_into().unwrap(), to_check_y.try_into().unwrap());
